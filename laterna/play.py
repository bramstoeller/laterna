#!/usr/bin/env python
"""Play the scenes from scenes.yaml on the projector.

Keys:
  Enter / Space / right arrow   next scene
  Backspace / left arrow   previous scene
...
the scene keys (Enter, space, the arrows) leave the slideshow and move
between scenes as always.

The slideshows and videos inside a scene get their own bars in the top
LEFT corner, one row per distinct timing in the order the scene maps
...
A scene with `hold: <seconds>` (its own or the global default at the top
of scenes.yaml) moves on to the next scene by itself that long after its
...
blackout scene waiting for a key goes on to the next scene when the
...
entries between scenes in scenes.yaml; where absent, the global `fade`
...
The first scene is shown as soon as it is ready; the remaining scenes render
in a background thread (the border molding is computed only once). A small
progress line shows until everything is rendered; stepping to a scene that
...
when its scene is entered, except when the previous scene was already
playing the same file: then it runs on seamlessly.

A scene with `blackout: true` is all black. The fade into it (and out
...
# rendered scenes on disk, so a restart skips rendering
...
# going: (thread, stop event). Quitting to the menu while scenes were still
...
def _scene_file(i):
    return CACHE_DIR / f'scene-{i:02d}.png'


def _alpha_file(i):
    return CACHE_DIR / f'scene-{i:02d}-alpha.png'


def _is_blackout(scene):
    return bool(scene.get('blackout'))


def _molding_key(scene):
    """What a scene's molding render depends on: its molding_color (None =
    the default gold), or 'blackout' (all black, never cached)."""
    return 'blackout' if _is_blackout(scene) else scene.get('molding_color')
...
def _molding_keys(scenes):
    """The distinct molding renders the scenes need, in order of first use."""
    return list(dict.fromkeys(_molding_key(s) for s in scenes if not _is_blackout(s)))


def _has_video(scene):
    """True when the scene needs per-tick compositing (video or slideshow)."""
...
def _manifest_valid(fp, scenes):
    ...
    return (
        ...
        and data.get('scenes') == total
        and all(_scene_file(i).exists() for i in range(total))
        and all(_alpha_file(i).exists() for i, s in enumerate(scenes) if _has_video(s))
        and all(_molding_file(k).exists() for k in _molding_keys(scenes))
    )
...
def _clear_cache():
    ...
    or mismatched, or any scene file gone), so no leftovers from an older
    show can linger.
    """
    if not CACHE_DIR.exists():
        return
    for pattern in ('scene-*.png', 'molding-*.png'):
...
def run(
    ...
    if not scenes:
        sys.exit('no scenes in scenes file')
    total = len(scenes)
    desk = dmx.Desk(cfg)
    ...
    pygame.key.set_repeat()  # no repeat: one scene per key press
    font = ui.help_font()
    ...
    fp = _fingerprint(config, scenes_path, cfg, scenes, supersample)
    images = {}  # scene index -> static render (video polygons black)
    alphas = {}  # scene index -> alpha layer, only for scenes with video
    moldings = {}  # molding key -> molding-only render (splits pictures from molding)
    players = {}  # scene index -> video.ScenePlayer, created lazily
    clip_pool = {}  # path -> VideoClip, shared so timelines survive scene changes
    idx = 0

    if _manifest_valid(fp, scenes):
        # valid cached renders: skip rendering
        print('cached renders match')
        for key in _molding_keys(scenes):
        ...
        for i, scene in enumerate(scenes):
            ...
            data = cv2.imread(str(_scene_file(i)), cv2.IMREAD_COLOR)
            images[i] = cv2.cvtColor(data, cv2.COLOR_BGR2RGB)
    else:
        # first scene synchronously (incl. the one-off border shading), the
        ...
        def render_scene(i):
            """Render + cache scene i. The alpha goes in before images[i]:
            its presence is what marks the scene as available."""
            image = renderer.render(scenes[i])
            render.save_png(image, _scene_file(i))
            alpha = renderer.video_alpha(scenes[i])
            if alpha is not None:
                cv2.imwrite(str(_alpha_file(i)), alpha)
                alphas[i] = alpha
            key = _molding_key(scenes[i])
            if key != 'blackout' and key not in moldings:
                moldings[key] = renderer.render_molding(scenes[i])
                render.save_png(moldings[key], _molding_file(key))
            images[i] = image
            print(f'  {i + 1}/{total}: {scenes[i].get("name", "?")}')
    ...
    def get_player(i):
        """Player of scene i, created on first use (None while the scene is
        still rendering). Creating one composites all slides of its
        slideshows, so prepare() does it ahead of time; otherwise it would
        happen at the start of the crossfade into the scene."""
        if i not in players and i in images:
            players[i] = video.ScenePlayer(cfg, scenes[i], images[i], alphas.get(i), clip_pool)
        return players.get(i)

    def prepare(i):
        if any('slideshow' in m for m in scenes[i].get('mappings', [])):
            get_player(i)
    ...
    shown = None  # the levels last drawn: a static scene redraws when the desk moves

    def step_slide(delta):
        """Step the current scene's slideshows one picture on or back."""
    ...
    def clip_states(i):
        """[(a fade is running, seconds left, the whole stretch)] for scene
        i's slideshows and videos, in the order the scene maps them: a bar
    ...
    def overlay():
        ...
        if transition_until is not None:  # the scene, top right
            ...
            waits_for_desk = master_dark and _is_blackout(scenes[idx]) and idx + 1 < total
    ...
    def wait_for(i):
        """Block until the background render delivers scene i."""
        if frame_for(i) is None:
            ui.draw_help(screen, font, [f'waiting for scene {i + 1}...'])
            pygame.display.flip()
    ...
    hold_until = None  # wall-clock seconds at which the scene's hold ends
    ...
    def arm_hold():
        ...
        nonlocal hold_until
        master_seen.clear()  # a new scene: the master starts over
        hold = scenes[idx].get('hold')
    ...
    def go_to(new, instant=False):
        """Move to scene `new`, fading (`instant`: at once). A double press
    ...
    if test:
        if total > 1:
            go_to(1)
        vi = next((i for i in range(total) if _has_video(scenes[i])), None)
        if vi is not None and wait_for(vi):
            ...
            print(
                f'animated scene "{scenes[vi].get("name", "?")}": '
    ...
    # a scene with video redraws continuously; a static scene sleeps on the
    ...
    while running:
        ...
        if player is not None and player.animated:
            ...
            # a static scene under the desk (or the faders): look every
            ...
            # sleep on the event queue, at most until the scene's hold ends
...
def main():
    ap = argparse.ArgumentParser(description='Play the scenes on the projector')
    ap.add_argument('--config', default='config.yaml')
    ap.add_argument('--scenes', default='scenes.yaml')
    ap.add_argument('--supersample', type=int, default=3)
    ap.add_argument(
        '--test', action='store_true', help='self-test: show first scene, one fade, quit'
    )
    ap.add_argument(
        '--dmx',
        choices=dmx.SOURCES,
